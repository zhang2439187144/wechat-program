...
var config = {
  ...
  colors: ['#7cb5ec', '#f7a35c', '#8085e9', '#90ed7d', '#f15c80', '#434348'],
  pieChartLinePadding: 20,
  pieChartTextPadding: 25,
  xAxisTextPadding: 3,
  titleColor: '#333333',
  titleFontSize: 20,
...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  textObjectCollection.forEach(function(item) {
    ...
    context.setLineWidth(1);
    context.setFontSize(config.fontSize - 5); //饼图的数据值大小
    context.beginPath();
    ...
    context.beginPath();
    context.setFillStyle('#666666');
    context.fillText(item.text, textStartX, textPosition.y + 3); //饼图的数据值
    context.closePath();
    context.stroke();
...
function drawYAxisTitle(title, opts, config, context) {
  ...
  context.beginPath();
  context.setFontSize(config.fontSize);
  context.setFillStyle(opts.yAxis.titleFontColor || '#00FFFF');
  ...
  legendList.forEach(function(itemList, listIndex) {
    ...
    itemList.forEach(function(item) {
      switch (opts.type) {
        ...
        case 'pie':
        case 'ring':
          if (ccount > 2 && listline == 1) {
            startY += 40;
            listline = 2;
            startX = 30;
          }
          if (ccount > 4 && listline == 2) {
            startY += 40;
            listline = 4;
            startX = 30;
          }
          if (ccount > 6 && listline == 4) {
            startY += 40;
            listline = 6;
            startX = 30;
          }
          context.beginPath();
          context.setFillStyle(item.color); //饼图的圆点绘制
          context.moveTo(startX, startY); //形状宽度除以二==半径
          context.arc(startX + 7.5, startY, shapeWidth / 2, 0, 2 * Math.PI);
          context.closePath();
          context.fill();
          break;
        default:
      ...
      startX += shapeWidth * 1.2; ////饼图提示数据字体开始的x位置
      context.beginPath();
      context.setFillStyle(opts.extra.legendTextColor || '#333333');
      context.fillText(item.name, startX, startY + 9); //饼图提示数据的绘制
      context.closePath();
      context.stroke();
      startX += measureText(item.name) * 2.5 + padding;
      ///stdrtX += item.name.length*config.fontSize+padding;//改良后的计算
